...
Item {
    ...
    // v0.14.7 — synchronisation facultative de la carte avec les lignes filtrées.
    ...
    property var mapFilterLayer: null
    property string mapFilterOriginalSubset: ""
    property string mapFilterAppliedSubset: ""
    property string mapFilterStatus: ""
    property string mapFilterDiagnostic: ""
...
property int totalFeatureCount: 0
    ...
    property bool loadAllRecords: true
    ...
    // v0.14.7 — Autosauvegarde du formulaire ouvert par QField Table.
    ...
    // v0.14.7 — sélection et modification en lot.
    ...
    property bool batchClearRelationValue: false
    property bool batchClearSimpleValue: false
    property int batchSuccessCount: 0
    property var batchFailedIds: []
    property bool batchInProgress: false
    // v0.14.7 — source ValueRelation complète.
    ...
    // v0.14.7 — le FeatureModel de schéma est détaché/rattaché
    ...
    // v0.14.7 : cache des libellés ValueRelation / ValueMap.
    ...
    // v0.14.7 — vues partagées synchronisées via une table du GeoPackage.
    property string sharedViewsLayerName: "qfield_table_vues"
    property var sharedViewsLayer: null
    property string sharedViewsError: ""
    // v0.14.7 — préférences locales de démarrage (non synchronisées).
    property string startupSharedViewUuid: ""
    property string startupSharedViewTitle: ""
    property bool startupMapFilterEnabled: false
    property bool startupViewPending: false
    // v0.14.7 — état courant de travail, strictement local à l'appareil.
    property string sessionLayerName: ""
    property var sessionFilters: []
    property string sessionGlobalSearch: ""
    property bool sessionMapFilterEnabled: false
    property bool sessionRestorePending: false
    property bool sessionPersistenceReady: false
    ...
    function refreshLayers() {
        ...
        } catch (e1) { console.log("QField Table v0.14.7 mapLayers: " + e1) }
        ...
        } catch (e2) { console.log("QField Table v0.14.7 canvas layers: " + e2) }
    ...
    function selectLayer(index) {
        ...
        // Ne jamais laisser un filtre cartographique temporaire sur l'ancienne couche.
        restoreMapDisplay()
    ...
    function inspectSelectedLayer() {
        ...
        // Un filtre restauré par StartupFilter existe avant même l'ouverture
        // de QFieldTable. Le retirer avant le premier parcours permet à la
        // table de retrouver la couche complète.
        clearExternalMapFilter(false)
        ...
        try {
            ...
            console.log("QField Table v0.14.7 iterator: " + error)
        }
    ...
    function reloadFeaturesOnly() {
        ...
        // Le filtre de carte agit au niveau du fournisseur. On le retire avant
        // de reconstruire la table afin que QField Table conserve toujours
        // l'ensemble des données comme source de filtrage.
        var reapplyMapFilter = mapFilterEnabled
        if (mapFilterActive)
            restoreMapDisplay(false)
        else
            clearExternalMapFilter(false)
        ...
        try {
            ...
            console.log("QField Table v0.14.7 filtered iterator: " + error)
        }
        updateLoadStatus()
    ...
    function representedValue(feature, fieldName, rawValue) {
        ...
        try {
            ...
            console.log("QField Table v0.14.7 represent_value(" + fieldName + "): " + e)
    ...
    function optimizeColumnWidths(rows) {
        // v0.14.7 : ne parcourt plus toutes les cellules au chargement.
    ...
    function parseStoredConfigurations() {
        try {
            ...
            console.log("QField Table v0.14.7 configuration invalide: " + e)
            return ({})
        }
    }
    ...
    function readLayerConfiguration() {
        ...
        try {
            ...
            console.log("QField Table v0.14.7 lecture propriété couche: " + e)
        }
        return null
    }
    ...
    function saveColumnConfiguration() {
        ...
        try {
            ...
            console.log("QField Table v0.14.7 sauvegarde propriété couche: " + e)
        }
    }
    ...
    function rowMatchesGlobalSearch(row) {
        var term =
                searchField
                ? String(searchField.text).toLowerCase().trim()
                : ""

        if (term.length === 0)
            return true

        if (String(row.featureId).toLowerCase().indexOf(term) >= 0)
            return true

        for (var c = 0; c < columns.length; ++c) {
            if (String(rowValue(row, c))
                    .toLowerCase()
                    .indexOf(term) >= 0)
                return true
        }
    ...
    // Teste tous les filtres actifs sauf celui de la colonne indiquée.
    // C'est le principe d'un filtre facetté : lorsque l'utilisateur ouvre
    // le filtre de la colonne C, les compteurs de C reflètent A + B + D...
    // sans être réduits par l'ancien filtre de C lui-même.
    function rowMatchesFiltersExcept(row, excludedColumnIndex) {
        for (var k in activeColumnFilters) {
            var idx = Number(k)

            if (Number(idx) === Number(excludedColumnIndex))
                continue
            ...
            if (f.mode === "values" &&
                (!f.selected ||
                 f.selected[distinctKey(value)] !== true))
                return false
    ...
    function findSharedViewsLayer() {
        ...
        try {
            ...
            console.log("QField Table v0.14.7 recherche qfield_table_vues : " + e)
        }
    ...
    function saveSelectedViewAsStartup() {
        var item = selectedSharedView()
        if (!item) {
            sharedViewsError = qsTr("Aucune vue n’est sélectionnée.")
            return false
        }
        startupSharedViewUuid = String(item.uuid || "")
        startupSharedViewTitle = String(item.title || "")
        if (saveAutosaveSettings()) {
            sharedViewsError = qsTr("Vue de démarrage enregistrée localement sur cet appareil.")
            return true
        }
        sharedViewsError = qsTr("Impossible d’enregistrer la préférence locale.")
        return false
    }

    function clearStartupView() {
        startupSharedViewUuid = ""
        startupSharedViewTitle = ""
        startupMapFilterEnabled = false
        saveAutosaveSettings()
        sharedViewsError = qsTr("Vue de démarrage locale supprimée.")
    }

    function applyStartupSharedView() {
        if (!startupSharedViewUuid || startupSharedViewUuid.length === 0)
            return false
        if (!loadSharedViews())
            return false
        for (var i = 0; i < sharedViewsListModel.count; ++i) {
            var item = sharedViewsListModel.get(i)
            if (String(item.uuid || "") === startupSharedViewUuid) {
                sharedViewsCombo.currentIndex = i
                mapFilterEnabled = startupMapFilterEnabled
                return applySelectedSharedView()
            }
        }
        sharedViewsError = qsTr("La vue de démarrage locale n’existe plus dans qfield_table_vues.")
        return false
    }
    ...
    function buildProjectXmlDiagnostic(content, sourceDescription, registeredCount) {
        ...
        var lines = []
        lines.push("QField Table v0.14.7 — diagnostic projet")
    ...
    function parseProjectWidgetConfigsXml(content, sourceDescription) {
        ...
        // v0.14.7: ValueMap / Liste de valeurs is stored in the project too.
    ...
    function allProjectVectorLayers() {
        ...
        try {
            ...
            console.log("QField Table v0.14.7 ProjectUtils.mapLayers: " + e1)
        }
    ...
    function resolveProjectVectorLayer(layerIdValue, layerNameValue, layerSourceValue) {
        ...
        if (wantedId.length > 0) {
            try {
                ...
                console.log("QField Table v0.14.7 direct layer lookup: " + e0)
            }
        }
    ...
    function loadBatchRelationItemsWithIterator() {
        ...
        try {
            ...
            console.log(
                "QField Table v0.14.7 relation iterator: " + e
            )
        } finally {
            try {
                if (iterator)
        ...
        // v0.14.7 : une ValueRelation simple peut être explicitement remise
        ...
        if (!batchIsMultiValueRelation) {
            result.unshift({
                "rawValue": "__QFT_NULL__",
                "label": qsTr("Aucune valeur (NULL)"),
                "isNullChoice": true
            })
        }
    ...
    function writeBatchJournalEntryToLayer(entry) {
        ...
        try {
            ...
            // v0.14.7 : champs optionnels. Un ancien journal continue de
    ...
    function journalCloudUser() {
        try {
            ...
            console.log('QField Table v0.14.7 cloud user: ' + e)
        }
    ...
    function openBatchEditDialog() {
        ...
        batchClearRelationValue = false
        batchClearSimpleValue = false
        batchRelationItems = []
    ...
    function batchNewRawValue(row) {
        ...
        // ValueRelation unique : la valeur spéciale "Aucune valeur"
        // écrit réellement NULL dans le champ.
        if (batchIsValueRelation && !batchIsMultiValueRelation) {
            if (batchClearRelationValue)
                return null
        ...
        if (!fieldLooksMultiple(batchFieldColumn)) {
            if (batchClearSimpleValue)
                return null
            return batchValueText
        }

        // ValueRelation multiple : vider le tableau signifie conserver
        // un tableau valide mais vide, donc {} plutôt que NULL.
        if (batchIsValueRelation &&
            batchIsMultiValueRelation &&
            batchOperation === "empty")
            return "{}"
    ...
    function executeBatchEdit() {
        ...
        for (var i = 0; i < rows.length; ++i) {
            ...
            if ((newValue === null || newValue === undefined) &&
                !(batchIsValueRelation &&
                  !batchIsMultiValueRelation &&
                  batchClearRelationValue) &&
                !batchClearSimpleValue) {
            ...
            try {
                ...
                console.log("QField Table v0.14.7 batch feature " + row.featureId + ": " + e)
                batchFailedIds.push(String(row.featureId))
    ...
    function buildRows() {
        ...
        // v0.14.7 : la construction initiale ne lit plus chaque attribut de
        ...
        // Restaurer la dernière session seulement lorsque les colonnes et
        // les lignes existent réellement. Cela évite d'écraser les filtres
        // sauvegardés pendant l'initialisation du plugin.
        if (!restoreCurrentSessionState())
    ...
    function rebuildDistinctValues() {
        ...
        for (var i = 0; i < flatRows.length; ++i) {
            ...
            // v0.14.7 : le compteur doit représenter uniquement les
            // enregistrements encore disponibles après les AUTRES filtres.
            if (!rowMatchesGlobalSearch(row))
                continue

            if (!rowMatchesFiltersExcept(row, filterColumn))
                continue
    ...
    function pendingResultCount() {
        ...
        for (var r = 0; r < flatRows.length; ++r) {
            ...
            var key =
                    distinctKey(
                        rowValue(row, filterColumn))

            if (pendingDistinctKeys[key] === true)
                count++
        }
    ...
    function mapFilterPrimaryKeyField() {
        ...
        try {
            ...
            if (provider && provider.pkAttributeIndexes) {
                var indexes = provider.pkAttributeIndexes()
    ...
    function sqlQuotedField(name) {
        return '"' + String(name).replace(/"/g, '""') + '"'
    }

    function columnIndexForFieldName(fieldName) {
        var wanted = String(fieldName || "").toLowerCase()

        for (var i = 0; i < columns.length; ++i) {
            if (String(columns[i].fieldName || "").toLowerCase() === wanted)
                return i
        }
    ...
    function mapFilterValueForRow(row) {
        var fieldName = mapFilterPrimaryKeyField()
        var idx = columnIndexForFieldName(fieldName)

        // IMPORTANT : row.featureId est l'identifiant interne du fournisseur,
        // pas nécessairement la valeur du champ fid stocké dans le GeoPackage.
        if (idx >= 0) {
            var raw = rawAttributeForRow(row, idx)
            if (raw !== undefined && raw !== null && String(raw).length > 0)
                return raw
        }

        // Repli seulement lorsqu'aucun champ de clé n'est disponible.
        return row.featureId
    }

    function filteredFeatureIdSql() {
        var numericValues = []
        var textValues = []
        var fieldName = mapFilterPrimaryKeyField()

        for (var i = 0; i < filteredRows.length; ++i) {
            var value = mapFilterValueForRow(filteredRows[i])
            ...
            if (/^-?\d+$/.test(text))
                ...
                textValues.push("'" + text.replace(/'/g, "''") + "'")
        }

        var values = numericValues.concat(textValues)

        if (values.length === 0)
            return "1 = 0"

        return sqlQuotedField(fieldName) +
               " IN (" + values.join(",") + ")"
    }


    function currentLayerSubset() {
        if (!selectedLayer) return ""
        try {
            if (typeof selectedLayer.subsetString === "function")
                return String(selectedLayer.subsetString() || "")
            return String(selectedLayer.subsetString || "")
        } catch (e) {
            return ""
        }
    ...
    function setLayerSubset(layer, expression) {
        if (!layer) return false
        try {
            if (typeof layer.setSubsetString === "function")
                return layer.setSubsetString(expression)

            // QField expose également subsetString comme propriété QML.
            layer.subsetString = expression
            return true
        } catch (e) {
            diagnosticMessage = qsTr("Impossible de filtrer la carte : %1").arg(String(e))
            console.log("QField Table v0.14.7 subset: " + e)
            return false
        }
    }

    function buildMapFilterDiagnostic() {
        if (!selectedLayer) {
            mapFilterDiagnostic = qsTr("Aucune couche sélectionnée.")
            return
        }

        var lines = []
        lines.push("QField Table v0.14.7 — DIAGNOSTIC CARTE")
        lines.push("Couche : " + layerName(selectedLayer))
        lines.push("Champ choisi pour subset : " + mapFilterPrimaryKeyField())
        lines.push("Subset avant filtre : " + currentLayerSubset())
        lines.push("Lignes filtrées : " + filteredRows.length)
        lines.push("")

        var maxRows = Math.min(12, filteredRows.length)

        for (var i = 0; i < maxRows; ++i) {
            var row = filteredRows[i]
            var featureIdValue = String(row.featureId)
            var fidRaw = ""
            var uniqueRaw = ""
            ...
            try {
                var idxFid = -1
                for (var c = 0; c < columns.length; ++c) {
                    if (String(columns[c].fieldName).toLowerCase() === "fid") {
                        idxFid = c
                        break
                    }
                }
                if (idxFid >= 0)
                    fidRaw = String(rawAttributeForRow(row, idxFid))
            } catch (e1) {}

            try {
                var idxUnique = -1
                for (var u = 0; u < columns.length; ++u) {
                    if (String(columns[u].fieldName) === "id_unique_inv") {
                        idxUnique = u
                        break
                    }
                }
                if (idxUnique >= 0)
                    uniqueRaw = String(rawAttributeForRow(row, idxUnique))
            } catch (e2) {}

            lines.push(
                "[" + (i + 1) + "] featureId=" + featureIdValue +
                " | fid=" + fidRaw +
                " | clé utilisée=" + String(mapFilterValueForRow(row)) +
                " | id_unique_inv=" + uniqueRaw
            )
        }

        lines.push("")
        lines.push("Expression générée :")
        lines.push(filteredFeatureIdSql())

        mapFilterDiagnostic = lines.join("\n")
    }

    function generatedMapSubsetInfo(expression) {
        var text = String(expression || "").trim()

        if (text.length === 0)
            return ({ "matched": false, "original": "" })

        var generated = '(?:"(?:fid|FID|ogc_fid|OGC_FID)"\\s+IN\\s*\\([^)]*\\)|1\\s*=\\s*0)'
        var onlyGenerated = new RegExp('^' + generated + '$', 'i')
        if (onlyGenerated.test(text))
            return ({ "matched": true, "original": "" })

        // Forme combinée produite lorsque la couche possédait déjà un subset :
        // (<subset d'origine>) AND (<filtre cartographique>)
        var combined = new RegExp('^\\((.*)\\)\\s+AND\\s+\\((' + generated + ')\\)$', 'i')
        var match = combined.exec(text)
        if (match)
            return ({ "matched": true, "original": String(match[1] || "") })

        return ({ "matched": false, "original": "" })
    }

    function looksLikeLegacyQFieldTableSubset(expression) {
        return generatedMapSubsetInfo(expression).matched
    }

    function clearExternalMapFilter(showToast) {
        if (!selectedLayer)
            return false

        var info = generatedMapSubsetInfo(currentLayerSubset())
        if (!info.matched)
            return false

        var ok = setLayerSubset(selectedLayer, info.original)
        try { selectedLayer.triggerRepaint() } catch (e1) {}

        mapFilterActive = false
        mapFilterLayer = null
        mapFilterAppliedSubset = ""
        mapFilterOriginalSubset = ""
        mapFilterStatus = ""

        if (ok && showToast !== false) {
            try { mainWindow.displayToast(qsTr("Affichage complet de la carte restauré.")) }
            catch (e2) {}
        }
        return ok
    }

    function applyMapFilter(showToast) {
        if (!selectedLayer) return false

        buildMapFilterDiagnostic()

        // Première application : mémoriser exactement le subset préexistant.
        if (!mapFilterActive || mapFilterLayer !== selectedLayer) {
            if (mapFilterActive)
                restoreMapDisplay(false)

            mapFilterLayer = selectedLayer

            var existingSubset = currentLayerSubset()

            // Si une ancienne version du plugin a laissé son subset erroné,
            // ne pas le considérer comme un filtre initial légitime.
            if (looksLikeLegacyQFieldTableSubset(existingSubset))
                mapFilterOriginalSubset = ""
            else
                mapFilterOriginalSubset = existingSubset
        }

        var idsSql = filteredFeatureIdSql()
        var combined = idsSql

        if (mapFilterOriginalSubset.trim().length > 0)
            combined = "(" + mapFilterOriginalSubset + ") AND (" + idsSql + ")"

        if (!setLayerSubset(selectedLayer, combined)) {
            mapFilterActive = false
            mapFilterStatus = qsTr("Échec du filtre cartographique")
            return false
        }
        ...
        if (showToast !== false) {
            try {
                mainWindow.displayToast(
                    qsTr("Carte filtrée : %1 entité(s).").arg(filteredRows.length))
            } catch (e2) {}
        }
        return true
    }

    function restoreMapDisplay(showToast) {
        if (!mapFilterActive || !mapFilterLayer) {
            // Le filtre a pu être posé par StartupFilter avant que QFieldTable
            // ne soit ouvert; il n'est alors pas connu comme mapFilterActive.
            clearExternalMapFilter(showToast)
            mapFilterActive = false
            mapFilterAppliedSubset = ""
            mapFilterStatus = ""
            return true
        ...
        var layer = mapFilterLayer
        var original = mapFilterOriginalSubset
        var ok = setLayerSubset(layer, original)

        try { layer.triggerRepaint() } catch (e1) {}
    ...
    function setMapFilterEnabled(enabled) {
        ...
        if (enabled)
            ...
            restoreMapDisplay(true)

        saveCurrentSessionState()
    }

    function zoomToFilteredResults() {
        if (!selectedLayer || filteredRows.length === 0)
            return

        var wasEnabled = mapFilterEnabled
        var wasActive = mapFilterActive

        // setCenterToLayer utilise l'étendue du subset courant. Si la
        // synchronisation est désactivée, on applique donc le subset juste
        // le temps de calculer l'étendue puis on restaure l'affichage.
        if (!mapFilterActive && !applyMapFilter(false))
            return

        try {
            var canvas = iface.mapCanvas()
            if (canvas && canvas.mapSettings &&
                typeof canvas.mapSettings.setCenterToLayer === "function") {
                canvas.mapSettings.setCenterToLayer(selectedLayer, true)
            } else {
                diagnosticMessage = qsTr("La fonction de zoom sur la couche n'est pas disponible.")
            }
        } catch (e) {
            diagnosticMessage = qsTr("Impossible de zoomer sur les résultats : %1").arg(String(e))
        }
    ...
    function applyView() {
        ...
        // Lorsque la synchronisation est activée, chaque filtre successif de
        // la table est immédiatement répercuté sur la carte.
        if (mapFilterEnabled)
            applyMapFilter(false)

        // v0.14.7 — mémoriser automatiquement l'état courant.
        saveCurrentSessionState()
    }
    ...
    function zoomToFeature(featureIdValue) {
        ...
        try {
            ...
            console.log("QField Table v0.14.7 zoom: " + zoomError)
            return false
        }
    }
    ...
    function handOffToNativeQField(featureIdValue) {
        ...
        try {
            ...
            console.log("QField Table v0.14.7 sélection : " + selectionError)
        }
    ...
    function jumpToPendingNativeFeature() {
        ...
        try {
            ...
            console.log("QField Table v0.14.7 jumpTo : " + jumpError)
    ...
    function openNativeFeatureForm(feature) {
        ...
        try {
            ...
            console.log("QField Table v0.14.7 formulaire natif : " + formError)
    ...
    function loadAutosaveSettings() {
        ...
        try {
            ...
            startupSharedViewUuid = String(obj.startupSharedViewUuid || "")
            startupSharedViewTitle = String(obj.startupSharedViewTitle || "")
            startupMapFilterEnabled = obj.startupMapFilterEnabled === true

            var session = obj.currentSession || ({})
            sessionLayerName = String(session.layer || "")
            sessionFilters = Array.isArray(session.filters) ? session.filters : []
            sessionGlobalSearch = String(session.globalSearch || "")
            sessionMapFilterEnabled = session.mapFilterEnabled === true
            sessionRestorePending =
                    sessionLayerName.length > 0 &&
                    (sessionFilters.length > 0 ||
                     sessionGlobalSearch.length > 0 ||
                     sessionMapFilterEnabled)

            return true
        } catch (e) {
            console.log("QField Table v0.14.7 réglages autosave : " + e)
            return false
        }
    }
    ...
    function saveAutosaveSettings() {
        ...
        try {
            var payload = {
                ...
                "startupSharedViewUuid": startupSharedViewUuid,
                "startupSharedViewTitle": startupSharedViewTitle,
                "startupMapFilterEnabled": startupMapFilterEnabled,
                "currentSession": {
                    "layer": sessionLayerName,
                    "filters": sessionFilters,
                    "globalSearch": sessionGlobalSearch,
                    "mapFilterEnabled": sessionMapFilterEnabled
                }
            }
            ...
            console.log("QField Table v0.14.7 sauvegarde réglages : " + e)
            return false
        }
    }
    ...
    function serializeCurrentFilters() {
        ...
        for (var k in activeColumnFilters) {
            ...
            fs.push({
                "field": String(columns[idx].fieldName || ""),
                "mode": String(f.mode || "values"),
                "text": String(f.text || ""),
                "values": vals
            })
    ...
    function saveCurrentSessionState() {
        if (!sessionPersistenceReady || !selectedLayer || columns.length === 0)
            return false

        sessionLayerName = layerName(selectedLayer)
        sessionFilters = serializeCurrentFilters()
        sessionGlobalSearch = searchField ? String(searchField.text || "") : ""
        sessionMapFilterEnabled = mapFilterEnabled === true
        return saveAutosaveSettings()
    }

    function restoreCurrentSessionState() {
        if (!sessionRestorePending || !selectedLayer || columns.length === 0)
            return false

        if (sessionLayerName.length > 0 &&
            layerName(selectedLayer) !== sessionLayerName)
            return false

        var indexByName = ({})
        for (var i = 0; i < columns.length; ++i)
            indexByName[String(columns[i].fieldName || "")] = i

        var restored = ({})
        for (var j = 0; j < sessionFilters.length; ++j) {
            var f = sessionFilters[j] || ({})
            var idx = indexByName[String(f.field || "")]
            if (idx === undefined)
                continue
        ...
        activeColumnFilters = restored
        filterColumn = -1
        filterText = ""
        selectedDistinctKeys = ({})

        if (searchField)
            searchField.text = sessionGlobalSearch
    ...
    function scrollNativeFormByWheel(wheel) {
        ...
        try {
            ...
            console.log(
                "QField Table v0.14.7 défilement formulaire : " +
                scrollError)
            return false
        }
    }
    ...
    function nativeAutosaveCurrentForm() {
        ...
        if (nativeFormIsNewEntity()) {
            console.log("QField Table v0.14.7 Autosave : nouvelle entité ignorée")
            return
        }
        ...
        try {
            if (model &&
                ...
                console.log("QField Table v0.14.7 Autosave : contrainte invalide")
                return
            }
        } catch (constraintError) {
            console.log("QField Table v0.14.7 Autosave contraintes : " +
                        constraintError)
        }
        ...
        try {
            ...
            if (ok) {
                ...
                console.log("QField Table v0.14.7 Autosave : sauvegarde réussie")
            } else {
                console.log("QField Table v0.14.7 Autosave : sauvegarde refusée")
            }

        } catch (saveError) {
            console.log("QField Table v0.14.7 Autosave erreur : " + saveError)
        }
    }
    ...
    function nativeAutosaveValueChanged() {
        ...
        console.log("QField Table v0.14.7 Autosave : modification détectée")
    }
    ...
    function saveNativeFeatureFormCore(showToast) {
        ...
        try {
            ...
            console.log("QField Table v0.14.7 sauvegarde : " + saveError)
    ...
    function closeNativeFeatureFormAndReturn() {
        ...
        try {
            ...
            console.log(
                "QField Table v0.14.7 fermeture après sauvegarde : " +
                closeError)
    ...
    function cancelNativeFeatureForm() {
        ...
        try {
            if (drawer.featureForm &&
                drawer.featureForm.requestCancel)
                drawer.featureForm.requestCancel()
            else
                drawer.close()

        } catch (cancelError) {
            console.log("QField Table v0.14.7 annulation : " + cancelError)
    ...
    function collectNativeEditDiagnostic(feature) {
        ...
        var text = "QField Table v0.14.7 — diagnostic formulaire natif\n\n"
    ...
    function openBrowser() {
        ...
        if (startupViewPending) {
            ...
            Qt.callLater(function() { plugin.applyStartupSharedView() })
        }
    }
    ...
    Component.onCompleted: {
        Qt.callLater(function() {
            ...
            startupViewPending = startupSharedViewUuid.length > 0
        ...
        console.log("QField Table v0.14.7 chargé")
    }

    Component.onDestruction: {
        // Sauvegarder l'état logique avant de retirer le subset temporaire.
        saveCurrentSessionState()
        // Ne jamais laisser un subset temporaire dans le projet après
        // désactivation ou rechargement du plugin.
        restoreMapDisplay(false)
    }
    ...
    Connections {
        ...
        function onCancelled() {
            // Rien à sauvegarder, mais le tableau sera relu à sa prochaine
            // ouverture pour rester cohérent avec QField.
        }
    }
    ...
    Dialog {
        id: mapFilterDiagnosticDialog
        parent: mainWindow.contentItem
        modal: true
        width: Math.min(mainWindow.width * 0.82, 980)
        height: Math.min(mainWindow.height * 0.72, 700)
        anchors.centerIn: parent
        title: qsTr("Diagnostic du filtre cartographique")
        ...
        contentItem: ColumnLayout {
            ...
            Label {
                ...
                text: qsTr("Compare featureId et fid sur les premières lignes filtrées.")
            }
            ...
            ScrollView {
                ...
                TextArea {
                    text: plugin.mapFilterDiagnostic
                    readOnly: true
    ...
    QfDialog {
        ...
        parent: mainWindow.contentItem
        modal: true
        title: qsTr("QField Table — v0.14.7")
        standardButtons: Dialog.Close
        ...
        contentItem: ColumnLayout {
            ...
            RowLayout {
                Button {
                    text: plugin.mapFilterEnabled
                          ? qsTr("🗺 Carte synchronisée")
                          : qsTr("🗺 Filtrer la carte")
                    checkable: true
                    checked: plugin.mapFilterEnabled
                    enabled: plugin.selectedLayer !== null
                    onClicked: plugin.setMapFilterEnabled(checked)
                }

                Button {
                    text: qsTr("Afficher toute la carte")
                    enabled: plugin.mapFilterActive || plugin.mapFilterEnabled
                    onClicked: {
                        plugin.mapFilterEnabled = false
                        plugin.restoreMapDisplay(true)
                        plugin.saveCurrentSessionState()
                    }
                }

                Button {
                    text: qsTr("Zoomer sur les résultats")
                    enabled: plugin.filteredRows.length > 0
                    onClicked: plugin.zoomToFilteredResults()
                }

                Label {
                    Layout.fillWidth: true
                    text: plugin.mapFilterEnabled
                          ? plugin.mapFilterStatus
                          : qsTr("La carte n'est pas liée aux filtres de la table.")
                    opacity: 0.72
            ...
            // v0.14.7 : ListView virtualisé. Contrairement au Repeater des versions
    ...
    QfDialog {
        ...
        contentItem: ColumnLayout {
            ...
            Label {
                Layout.fillWidth: true
                wrapMode: Text.WordWrap
                text: qsTr("La table contient %1 enregistrement(s). Tous les enregistrements sont chargés par défaut. Le préfiltre permet au besoin de charger seulement un sous-ensemble.").arg(plugin.totalFeatureCount)
            }

            RowLayout {
                Layout.fillWidth: true
                Button {
                    text: qsTr("Recharger tous (%1)").arg(plugin.totalFeatureCount)
            ...
            Label {
                ...
                wrapMode: Text.WordWrap
                opacity: 0.7
                text: qsTr("Exemple : choisir Municipalité + Égale + 38065 recharge uniquement les enregistrements correspondants. Sans préfiltre, l’ensemble de la couche est chargé.")
            }
    ...
    QfDialog {
        ...
        onOpened: {
            ...
            batchOperationCombo.currentIndex =
                ...
                plugin.batchOperation === "replaceall" ? 2 :
                plugin.batchOperation === "empty" ? 3 : 0
            batchRelationCombo.currentIndex = -1
        ...
        contentItem: ColumnLayout {
            ...
            ComboBox {
                ...
                model: [
                    ...
                    qsTr("Vider le tableau ({})")
                ]
                onActivated: {
                    plugin.batchOperation =
                        currentIndex === 1 ? "remove" :
                        currentIndex === 2 ? "replaceall" :
                        currentIndex === 3 ? "empty" : "add"
                }
            }
            ...
            RowLayout {
                ...
                ComboBox {
                    ...
                    function synchronizeRawValue() {
                        if (currentIndex >= 0 &&
                            currentIndex < plugin.batchRelationItems.length) {
                            var item = plugin.batchRelationItems[currentIndex]
                            plugin.batchClearRelationValue =
                                item.isNullChoice === true ||
                                String(item.rawValue) === "__QFT_NULL__"
                            ...
                            plugin.batchClearRelationValue = false
                            plugin.batchRelationRawValue = ""
                        }
                    }
            ...
            ComboBox {
                ...
                onActivated: {
                    plugin.batchClearSimpleValue = false
                    synchronizeValue()
                }
            ...
            TextField {
                ...
                enabled: !plugin.batchClearSimpleValue
                onTextChanged: plugin.batchValueText = text
            }

            CheckBox {
                id: batchClearSimpleCheck
                Layout.fillWidth: true
                visible: plugin.batchFieldColumn >= 0 &&
                         !plugin.batchIsValueRelation &&
                         !plugin.fieldLooksMultiple(plugin.batchFieldColumn)
                text: qsTr("Vider le champ (NULL)")
                checked: plugin.batchClearSimpleValue
                onToggled: {
                    plugin.batchClearSimpleValue = checked
                    if (checked)
                        plugin.batchValueText = ""
                }
            }
        ...
        contentItem: ColumnLayout {
            ...
            Label {
                ...
                text: plugin.batchIsMultiValueRelation
                      ? (plugin.batchOperation === "empty"
                         ? qsTr("Opération : Vider le tableau\nNouvelle valeur stockée : {}")
                         : qsTr("Opération : %1\nValeur : %2")
                      ...
                      : (plugin.batchIsValueRelation
                         ? (plugin.batchClearRelationValue
                            ? qsTr("Nouvelle valeur : Aucune valeur (NULL)")
                            : qsTr("Nouvelle valeur : %1")
                              .arg(plugin.relationLabelForRaw(
                                   plugin.batchRelationRawValue.length > 0
                                   ? plugin.batchRelationRawValue
                                   : plugin.batchValueText)))
                         : (plugin.batchClearSimpleValue
                            ? qsTr("Nouvelle valeur : NULL")
                            : qsTr("Nouvelle valeur : %1")
    ...
    Dialog {
        ...
        contentItem: ColumnLayout {
            ...
            Rectangle {
                Layout.fillWidth: true
                implicitHeight: startupPrefsColumn.implicitHeight + 20
                radius: 6
                color: "transparent"
                border.width: 1
                ...
                ColumnLayout {
                    id: startupPrefsColumn
                    anchors.fill: parent
                    anchors.margins: 10
                    spacing: 6
                    Label { text: qsTr("Préférences locales sur cet appareil"); font.bold: true }
                    Label {
                        Layout.fillWidth: true
                        text: plugin.startupSharedViewTitle.length > 0
                              ? qsTr("Vue de démarrage : %1").arg(plugin.startupSharedViewTitle)
                              : qsTr("Aucune vue de démarrage définie")
                        wrapMode: Text.WordWrap
                    }
                    CheckBox {
                        text: qsTr("Filtrer aussi la carte au démarrage")
                        checked: plugin.startupMapFilterEnabled
                        enabled: plugin.startupSharedViewUuid.length > 0
                        onToggled: {
                            plugin.startupMapFilterEnabled = checked
                            plugin.saveAutosaveSettings()
                        }
                    }
                    RowLayout {
                        Button {
                            text: qsTr("Utiliser comme vue de démarrage")
                            enabled: sharedViewsListModel.count > 0
                            onClicked: plugin.saveSelectedViewAsStartup()
                        }
                        Button {
                            text: qsTr("Supprimer la vue de démarrage")
                            enabled: plugin.startupSharedViewUuid.length > 0
                            onClicked: plugin.clearStartupView()
                        }
                    }
                }
            }
